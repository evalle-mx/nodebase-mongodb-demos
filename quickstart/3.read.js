...
async function main() {
  ...
  const uri =
    ...
    `mongodb+srv://${process.env.username}:${process.env.password}@${process.env.cluster}/sample_airbnb?retryWrites=true&w=majority`;
...
async function findOneListingByName(client, nameOfListing) {
  ...
  if (result) {
    console.log(
      `__>>>__ Found a listing in the collection with the name '${nameOfListing}':`
    );
    console.log(result);
  } else {
...
async function findListingsWithMinimumBedroomsBathroomsAndMostRecentReviews(
  ...
  if (results.length > 0) {
    console.log(
      `__>>>__ Found listing(s) with at least ${minimumNumberOfBedrooms} bedrooms and ${minimumNumberOfBathrooms} bathrooms:`
    );
